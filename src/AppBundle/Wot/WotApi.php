...
Class WotApi
{
    ...
    public function getEvent($request, $search_value, $fields = array())
    {
        $u = $this->api_url.'/wot/globalmap/';
        ...
        switch ($request) {
            case 'search':
                ...
                break;
            case 'accountinfo':
                $u .= 'eventaccountinfo/?event_id='.$search_value['event_id'].
                    '&front_id='.$search_value['front_id'].
                    '&account_id='.$search_value['account_id'].'&'
                ;
                break;
            default:
                throw new InvalidArgumentException('Invalid value for parameter $request given: ' . $request);
